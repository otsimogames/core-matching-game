...
export default class Hint {
  ...
  handTween() {
    ...
    this.arrow.scale.set(otsimo.game.height / 768);
    const t = otsimo.game.add.tween(this.arrow).to({ y: this.answer.world.y }, otsimo.kv.game.hint_hand_duration, Phaser.Easing.Sinusoidal.Out, false);
    const t2 = otsimo.game.add.tween(this.arrow)
    ...
    t.chain(t2);
    t.start();
    const delay = 2 * otsimo.kv.game.hint_hand_duration;
    this.call(delay);
  ...
  handMatch() {
    ...
    const t = otsimo.game.add.tween(this.arrow).to({ y: this.match.world.y }, otsimo.kv.game.hint_hand_duration, Phaser.Easing.Sinusoidal.Out, false);
    const t2 = otsimo.game.add.tween(this.arrow.scale).to({ x: 0.8, y: 0.8 }, 200, Phaser.Easing.Back.Out, false);
    const t3 = otsimo.game.add.tween(this.arrow)
    ...
    t2.chain(t3);
    t.start();
    const delay = otsimo.kv.game.hint_hand_duration + otsimo.kv.game.hint_hand_match_duration + 100;
    this.call(delay);
    this.answer.tweenArray = this.tweenArr;
  }
